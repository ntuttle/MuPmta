...
class FORMS {
	...
	public function StartForm($id, $class, $width = false) {
		$this->id                       = $id;
		$id                             = "id=\"$id\"";
		if ($class === false) {$class   = 'col-xs-12';}
...
\"":'';
	...
	public function JS($javascript) {
		if (preg_match('/^http:\/\//', $javascript, $x)) {
			...
			$this->FORM[$this->id][] = "<script type=\"text/javascript\">$(document).ready(function(){
{$javascript}
      });</script>";
		}
	}
	...
	public function Select($name, $options, $value = false, $label = true, $null = true, $width = false, $MULTI = false, $id = false) {
		...
		if ($MULTI !== false) {$SIZE         = " size=\"".$MULTI."\" "; $MULTI         = 'multiple';} else { $SIZE         = false;}
		if (is_numeric($width)) {$width      = "style=\"width:{$width}px\"";} elseif ($width) {$CLASS      = $width; $width      = " class=\"form-control col-xs-12\" ";}
		$id                                  = empty($id)?$name:$id;
...
{$height}\"";
	...
	public function Text($N, $V = false, $L = true, $W = false, $ToolTip = false, $mid = false) {
		...
		if (!$V) {$V                 = @$_POST[$N];}
		$CLASS                       = '';
		if (is_numeric($W)) {$W      = " style=\"width:{$W}px\"";} elseif ($W) {$CLASS      = $W; $W      = " class=\"form-control col-xs-12\"";}
		$mid                         = empty($mid)?$N:$mid;
		$ID                          = " id=\"{$mid}\"";
		$T                           = " type=\"text\"";
